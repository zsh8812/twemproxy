 ...
 * return false
 */
static bool redis_argz(const struct msg* r) {
    switch (r->type) {
 ...
 * return false
 */
static bool redis_arg0(const struct msg* r) {
    switch (r->type) {
 ...
 * return false
 */
static bool redis_arg1(const struct msg* r) {
    switch (r->type) {
 ...
 * return false
 */
static bool redis_arg2(const struct msg* r) {
    switch (r->type) {
 ...
 * return false
 */
static bool redis_arg3(const struct msg* r) {
    switch (r->type) {
 ...
 * return false
 */
static bool redis_argn(const struct msg* r) {
    switch (r->type) {
        ...
        case MSG_REQ_REDIS_BITFIELD:
            /* TODO: Support REDIS_BITOP operation destkey key ... and add tests - this requires
             * handling key in a position other than the first one */
 ...
 */
static bool redis_argx(const struct msg* r) {
    switch (r->type) {
 ...
 */
static bool redis_argkvx(const struct msg* r) {
    switch (r->type) {
 ...
 */
static bool redis_argeval(const struct msg* r) {
    switch (r->type) {
...
static bool redis_nokey(const struct msg* r) {
    switch (r->type) {
 ...
 */
static bool redis_error(const struct msg* r) {
    switch (r->type) {
        ...
        switch (state) {
                ...
                if (ch == ' ' || ch == CR) {
                    ...
                    switch (p - m) {
                        ...
                        case 8:
                             ...
                             * are disabled. Please check Redis logs for details about the
                             * error.\r\n" */
                        ...
                        case 11:
                            /* -MASTERDOWN Link with MASTER is down and slave-serve-stale-data is
                             * set to 'no'.\r\n */
                            if (str11cmp(
                                    m, '-', 'M', 'A', 'S', 'T', 'E', 'R', 'D', 'O', 'W', 'N')) {
            ...
            case SW_MULTIBULK:
                if (r->token == NULL) {
                    ...
                    if (r->is_top_level) {
                        /* For multiget responses, we may need to know the number of responses to
                         * combine them. */
                        r->narg = r->vlen;
                        r->narg_end = p;
                    }
 ...
 *
 * */
static rstatus_t redis_copy_bulk(struct msg* dst, struct msg* src) {
...
static rstatus_t redis_append_key(struct msg* r, const uint8_t* key, uint32_t keylen) {
    uint32_t len;
    struct mbuf* mbuf;
    uint8_t printbuf[32];
    struct keypos* kpos;
 ...
 */
static rstatus_t
redis_fragment_argx(struct msg* r, uint32_t nserver, struct msg_tqh* frag_msgq, uint32_t key_step) {
    struct mbuf* mbuf;
    struct msg** sub_msgs;
    uint32_t i;
    rstatus_t status;
...
rstatus_t redis_fragment(struct msg* r, uint32_t nserver, struct msg_tqh* frag_msgq) {
    ...
    switch (r->type) {
        ...
        case MSG_REQ_REDIS_UNLINK:
            ...
            /* TODO: MSETNX - instead of responding with OK, respond with 1 if all fragments respond
             * with 1 */
        case MSG_REQ_REDIS_MSET:
...
static rstatus_t redis_handle_auth_req(struct msg* req, struct msg* rsp) {
    ...
    valid = (keylen == pool->redis_auth.len) && (memcmp(pool->redis_auth.data, key, keylen) == 0);
    if (valid) {
...
rstatus_t redis_add_auth(struct context* ctx, struct conn* c_conn, struct conn* s_conn) {
    ...
    msg = msg_get(ctx->msg_list, c_conn, true, c_conn->redis);
    if (msg == NULL) {
        c_conn->err = errno;
        return NC_ENOMEM;
    }
...
void redis_post_connect(struct context* ctx, struct conn* conn, struct server* server) {
     ...
     */
    msg = msg_get(ctx->msg_list, conn, true, conn->redis);
    if (msg == NULL) {
        return;
    }
...
void redis_swallow_msg(struct conn* conn, struct msg* pmsg, struct msg* msg) {
    if (pmsg != NULL && pmsg->type == MSG_REQ_REDIS_SELECT && msg != NULL && redis_error(msg)) {
