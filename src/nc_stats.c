...
static struct stats_metric stats_pool_codec[] = {STATS_POOL_CODEC(DEFINE_ACTION)};

static struct stats_metric stats_server_codec[] = {STATS_SERVER_CODEC(DEFINE_ACTION)};
#undef DEFINE_ACTION

#define DEFINE_ACTION(_name, _type, _desc) {.name = #_name, .desc = _desc},
static const struct stats_desc stats_pool_desc[] = {STATS_POOL_CODEC(DEFINE_ACTION)};

static const struct stats_desc stats_server_desc[] = {STATS_SERVER_CODEC(DEFINE_ACTION)};
#undef DEFINE_ACTION
...
void stats_describe(void) {
    ...
    for (i = 0; i < NELEMS(stats_pool_desc); i++) {
        log_stderr("  %-20s\"%s\"", stats_pool_desc[i].name, stats_pool_desc[i].desc);
    }
    ...
    for (i = 0; i < NELEMS(stats_server_desc); i++) {
        log_stderr("  %-20s\"%s\"", stats_server_desc[i].name, stats_server_desc[i].desc);
    }
}
...
static rstatus_t stats_server_init(struct stats_server* sts, struct server* s) {
    rstatus_t status;
    ...
    log_debug(LOG_VVVERB,
              "init stats server '%.*s' with %" PRIu32 " metric",
              sts->name.len,
              sts->name.data,
              array_n(&sts->metric));
...
static rstatus_t stats_server_map(struct array* stats_server, const struct array* server) {
    rstatus_t status;
    uint32_t i, nserver;
...
static rstatus_t stats_pool_init(struct stats_pool* stp, const struct server_pool* sp) {
    rstatus_t status;
    ...
    log_debug(LOG_VVVERB,
              "init stats pool '%.*s' with %" PRIu32 " metric and "
              "%" PRIu32 " server",
              stp->name.len,
              stp->name.data,
              array_n(&stp->metric),
...
static rstatus_t stats_pool_map(struct array* stats_pool, const struct array* server_pool) {
    rstatus_t status;
    uint32_t i, npool;
...
static rstatus_t stats_create_buf(struct stats* st) {
    ...
    if (st->buf.data == NULL) {
        log_error("create stats buffer of size %zu failed: %s", size, strerror(errno));
        return NC_ENOMEM;
    }
    st->buf.size = size;
...
static void stats_destroy_buf(struct stats* st) {
    if (st->buf.size != 0) {
...
stats_add_string(struct stats* st, const struct string* key, const struct string* val) {
    ...
    n = nc_snprintf(pos, room, "\"%.*s\":\"%.*s\", ", key->len, key->data, val->len, val->data);
    if (n < 0 || n >= ( int )room) {
        return NC_ERROR;
    }
...
static rstatus_t stats_add_num(struct stats* st, const struct string* key, int64_t val) {
    struct stats_buffer* buf;
    uint8_t* pos;
    size_t room;
    int n;
    ...
    n = nc_snprintf(pos, room, "\"%.*s\":%" PRId64 ", ", key->len, key->data, val);
    if (n < 0 || n >= ( int )room) {
        return NC_ERROR;
    }
...
static rstatus_t stats_add_footer(struct stats* st) {
    struct stats_buffer* buf;
    uint8_t* pos;
...
static rstatus_t stats_begin_nesting(struct stats* st, const struct string* key) {
    struct stats_buffer* buf;
    uint8_t* pos;
    size_t room;
    int n;
...
static rstatus_t stats_end_nesting(struct stats* st) {
    struct stats_buffer* buf;
    uint8_t* pos;
...
static void stats_aggregate_metric(struct array* dst, const struct array* src) {
    uint32_t i;
...
static void stats_aggregate(struct stats* st) {
    ...
    if (st->aggregate == 0) {
        log_debug(LOG_PVERB,
                  "skip aggregate of shadow %p to sum %p as "
                  "generator is slow",
                  st->shadow.elem,
                  st->sum.elem);
        return;
    ...
    log_debug(LOG_PVERB, "aggregate stats shadow %p to sum %p", st->shadow.elem, st->sum.elem);
...
static rstatus_t stats_make_rsp(struct stats* st) {
    rstatus_t status;
    uint32_t i;
...
static rstatus_t stats_listen(struct stats* st) {
    ...
    if (status < 0) {
        log_error("set reuseport on m %d failed for stats server: %s", st->sd, strerror(errno));
        return NC_ERROR;
    }

    status = bind(st->sd, ( struct sockaddr* )&si.addr, si.addrlen);
    if (status < 0) {
        log_error("bind on m %d to stats server addr '%.*s:%u' failed: %s",
                  st->sd,
                  st->addr.len,
                  st->addr.data,
                  st->port,
    ...
    if (status < 0) {
        log_error("listen on m %d for stats server '%.*s:%u' failed: %s",
                  st->sd,
                  st->addr.len,
                  st->addr.data,
                  st->port,
    ...
    log_debug(LOG_NOTICE,
              "m %d listening on stats server '%.*s:%u'",
              st->sd,
              st->addr.len,
              st->addr.data,
              st->port);
...
struct stats* stats_create(uint16_t stats_port,
                           const char* stats_ip,
                           int stats_interval,
                           const char* source,
                           const struct array* server_pool) {
    rstatus_t status;
    struct stats* st;
...
void stats_swap(struct stats* st) {
    ...
    log_debug(LOG_PVERB, "swap stats current %p shadow %p", st->current.elem, st->shadow.elem);
...
stats_pool_to_metric(struct context* ctx, const struct server_pool* pool, stats_pool_field_t fidx) {
    struct stats* st;
    ...
    log_debug(LOG_VVVERB, "metric '%.*s' in pool %" PRIu32 "", stm->name.len, stm->name.data, pidx);

    return stm;
}

void _stats_pool_incr(struct context* ctx,
    ...
    log_debug(LOG_VVVERB,
              "incr field '%.*s' to %" PRId64 "",
              stm->name.len,
              stm->name.data,
              stm->value.counter);
}

void _stats_pool_decr(struct context* ctx,
                      const struct server_pool* pool,
                      stats_pool_field_t fidx) {
    struct stats_metric* stm;
    ...
    log_debug(LOG_VVVERB,
              "decr field '%.*s' to %" PRId64 "",
              stm->name.len,
              stm->name.data,
              stm->value.counter);
}

void _stats_pool_incr_by(struct context* ctx,
                         const struct server_pool* pool,
                         stats_pool_field_t fidx,
                         int64_t val) {
    struct stats_metric* stm;
    ...
    log_debug(LOG_VVVERB,
              "incr by field '%.*s' to %" PRId64 "",
              stm->name.len,
              stm->name.data,
              stm->value.counter);
}

void _stats_pool_decr_by(struct context* ctx,
                         const struct server_pool* pool,
                         stats_pool_field_t fidx,
                         int64_t val) {
    struct stats_metric* stm;
    ...
    log_debug(LOG_VVVERB,
              "decr by field '%.*s' to %" PRId64 "",
              stm->name.len,
              stm->name.data,
              stm->value.counter);
}

void _stats_pool_set_ts(struct context* ctx,
                        const struct server_pool* pool,
                        stats_pool_field_t fidx,
                        int64_t val) {
    struct stats_metric* stm;
...
static struct stats_metric* stats_server_to_metric(struct context* ctx,
    ...
    log_debug(LOG_VVVERB,
              "metric '%.*s' in pool %" PRIu32 " server %" PRIu32 "",
              stm->name.len,
              stm->name.data,
              pidx,
              sidx);
...
void _stats_server_incr(struct context* ctx,
                        const struct server* server,
                        stats_server_field_t fidx) {
    struct stats_metric* stm;
...
void _stats_server_decr(struct context* ctx,
                        const struct server* server,
                        stats_server_field_t fidx) {
    struct stats_metric* stm;
...
void _stats_server_incr_by(struct context* ctx,
                           const struct server* server,
                           stats_server_field_t fidx,
                           int64_t val) {
    struct stats_metric* stm;
...
void _stats_server_decr_by(struct context* ctx,
                           const struct server* server,
                           stats_server_field_t fidx,
                           int64_t val) {
    struct stats_metric* stm;
...
void _stats_server_set_ts(struct context* ctx,
                          const struct server* server,
                          stats_server_field_t fidx,
                          int64_t val) {
    struct stats_metric* stm;
