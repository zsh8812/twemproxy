...
static pthread_rwlock_t rwlock; // = PTHREASD_RWLOCK_INITIALIZER;
...
void mbuf_insert(struct mhdr* mhdr, struct mbuf* mbuf) {
    ...
    log_debug(LOG_VVERB, "insert mbuf %p len %d", mbuf, ( int )(mbuf->last - mbuf->pos));
    pthread_rwlock_unlock(&rwlock);
}
...
void mbuf_remove(struct mhdr* mhdr, struct mbuf* mbuf) {
    pthread_rwlock_wrlock(&rwlock);
    log_debug(LOG_VVERB, "remove mbuf %p len %d", mbuf, ( int )(mbuf->last - mbuf->pos));
...
struct mbuf* mbuf_split(struct mhdr* h, uint8_t* pos, mbuf_copy_t cb, void* cbarg) {
    ...
    log_debug(LOG_VVERB,
              "split into mbuf %p len %" PRIu32 " and nbuf %p len "
...
void mbuf_init(const struct instance* nci) {
    ...
    log_debug(LOG_DEBUG,
              "mbuf hsize %d chunk size %zu offset %zu length %zu",
              ( int )MBUF_HSIZE,
              mbuf_chunk_size,
              mbuf_offset,
              mbuf_offset);
