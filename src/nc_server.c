...
static void server_resolve(struct server* server, struct conn* conn) {
    rstatus_t status;
...
struct server_pool* server_server_pool(struct conn* conn) {
    ASSERT(!conn->client && !conn->proxy);
    ASSERT(conn->owner != NULL);
    return (( struct server* )(conn->owner))->owner;
}
...
static rstatus_t server_each_set_owner(void* elem, void* data) {
...
rstatus_t server_init(struct array* server, struct array* conf_server, struct server_pool* sp) {
    ...
    log_debug(LOG_DEBUG,
              "init %" PRIu32 " servers in pool %" PRIu32 " '%.*s'",
              nserver,
              sp->idx,
              sp->name.len,
              sp->name.data);
...
static rstatus_t server_each_disconnect(void* elem, void* data) {
    struct server* server;
    struct server_pool* pool;
...
static void server_failure(struct context* ctx, struct server* server) {
    ...
    log_debug(LOG_VERB,
              "server '%.*s' failure count %" PRIu32 " limit %" PRIu32,
              server->pname.len,
              server->pname.data,
              server->failure_count,
    ...
    log_debug(LOG_INFO,
              "update pool %" PRIu32 " '%.*s' to delete server '%.*s' "
              "for next %" PRId64 " secs",
              pool->idx,
              pool->name.len,
              pool->name.data,
              server->pname.len,
    ...
    if (status != NC_OK) {
        log_error("updating pool %" PRIu32 " '%.*s' failed: %s",
                  pool->idx,
                  pool->name.len,
                  pool->name.data,
                  strerror(errno));
...
static void server_close_stats(struct context* ctx,
                               struct server* server,
                               err_t err,
                               unsigned eof,
                               unsigned connected) {
...
void server_close(struct context* ctx, struct conn* conn) {
    ...
    server_close_stats(ctx, conn->owner, conn->err, conn->eof, conn->connected);
    ...
    for (msg = TAILQ_FIRST(&conn->imsg_q); msg != NULL; msg = nmsg) {
        ...
        if (msg->swallow || msg->noreply) {
            log_debug(LOG_INFO,
                      "close s %d swallow req %" PRIu64 " len %" PRIu32 " type %d",
                      conn->sd,
                      msg->id,
                      msg->mlen,
                      msg->type);
            ...
            log_debug(LOG_INFO,
                      "close s %d schedule error for req %" PRIu64 " "
    ...
    if (msg != NULL) {
        ...
        log_debug(LOG_INFO,
                  "close s %d discarding rsp %" PRIu64 " len %" PRIu32 " "
                  "in error",
                  conn->sd,
                  msg->id,
                  msg->mlen);
...
rstatus_t server_connect(struct context* ctx, struct server* server, struct conn* conn) {
    ...
    log_debug(LOG_VVERB, "connect to server '%.*s'", server->pname.len, server->pname.data);

    conn->sd = socket(conn->family, SOCK_STREAM, 0);
    if (conn->sd < 0) {
        log_error("socket for server '%.*s' failed: %s",
                  server->pname.len,
                  server->pname.data,
                  strerror(errno));
        status = NC_ERROR;
...
static rstatus_t server_pool_update(struct server_pool* pool) {
    rstatus_t status;
    int64_t now;
    ...
    log_debug(LOG_INFO,
              "update pool %" PRIu32 " '%.*s' to add %" PRIu32 " servers",
              pool->idx,
              pool->name.len,
              pool->name.data,
...
server_pool_server(struct server_pool* pool, const uint8_t* key, uint32_t keylen) {
    ...
    log_debug(LOG_VERB,
              "key '%.*s' on dist %d maps to server '%.*s'",
              keylen,
              key,
              pool->dist_type,
              server->pname.len,
...
struct conn* server_pool_conn(struct context* ctx,
                              struct server_pool* pool,
                              const uint8_t* key,
                              uint32_t keylen) {
    rstatus_t status;
...
static rstatus_t server_pool_each_disconnect(void* elem, void* data) {
    rstatus_t status;
    struct server_pool* sp = elem;
...
static rstatus_t server_pool_each_set_owner(void* elem, void* data) {
    struct server_pool* sp = elem;
...
static rstatus_t server_pool_each_calc_connections(void* elem, void* data) {
    struct server_pool* sp = elem;
    struct context* ctx = data;
...
static rstatus_t server_pool_each_run(void* elem, void* data) {
    return server_pool_run(elem);
}

rstatus_t
server_pool_init(struct array* server_pool, struct array* conf_pool, struct context* ctx) {
    rstatus_t status;
    uint32_t npool;
...
void server_pool_deinit(struct array* server_pool) {
    ...
    for (i = 0, npool = array_n(server_pool); i < npool; i++) {
        ...
        log_debug(
            LOG_DEBUG, "deinit pool %" PRIu32 " '%.*s'", sp->idx, sp->name.len, sp->name.data);
    }
