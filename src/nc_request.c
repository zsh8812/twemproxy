...
struct msg* req_get(struct conn* conn) {
    ...
    msg = msg_get(msg_list, conn, true, conn->redis);
    if (msg == NULL) {
        conn->err = errno;
    }
    return msg;
...
static void req_log(const struct msg* req) {
    ...
    log_debug(LOG_NOTICE,
              "req %" PRIu64 " done on c %d req_time %" PRIi64 ".%03" PRIi64
              ...
              req_len,
              rsp_len,
              ( int )(kpos->end ? kpos->end - kpos->start : 0),
              kpos->start,
              peer_str,
              req->done,
              req->error);
...
bool req_done(const struct conn* conn, struct msg* msg) {
    ...
    for (cmsg = TAILQ_PREV(msg, msg_tqh, c_tqe); cmsg != NULL && cmsg->frag_id == id;
    ...
    for (cmsg = TAILQ_NEXT(msg, c_tqe); cmsg != NULL && cmsg->frag_id == id;
    ...
    log_debug(LOG_DEBUG,
              "req from c %d with fid %" PRIu64 " and %" PRIu32 " "
              "fragments is done",
              conn->sd,
              id,
              nfragment);
...
void req_server_enqueue_imsgq(struct context* ctx, struct conn* conn, struct msg* msg) {
     ...
     */
    if (!msg->noreply) {
        msg_tmo_insert(ctx->msg_list, msg, conn);
    }
...
static rstatus_t req_make_reply(struct context* ctx, struct conn* conn, struct msg* req) {
    struct msg* rsp;

    rsp = msg_get(ctx->msg_list, conn, false, conn->redis); /* replay */
    if (rsp == NULL) {
        conn->err = errno;
        return NC_ENOMEM;
    }
...
static bool req_filter(struct conn* conn, struct msg* msg) {
    ASSERT(conn->client && !conn->proxy);

    if (msg_empty(msg)) {
        ASSERT(conn->rmsg == NULL);
        log_debug(LOG_VERB, "filter empty req %" PRIu64 " from c %d", msg->id, conn->sd);
        req_put(msg);
        return true;
    }
     ...
     */
    if (msg->quit) {
        log_debug(LOG_INFO, "filter quit req %" PRIu64 " from c %d", msg->id, conn->sd);
        if (conn->rmsg != NULL) {
            log_debug(LOG_INFO,
                      "discard invalid req %" PRIu64 " len %" PRIu32 " "
...
static void req_forward_error(struct context* ctx, struct conn* conn, struct msg* msg) {
    rstatus_t status;

    ASSERT(conn->client && !conn->proxy);

    log_debug(LOG_INFO,
              "forward req %" PRIu64 " len %" PRIu32 " type %d from "
              "c %d failed: %s",
              msg->id,
              msg->mlen,
              msg->type,
...
static void req_forward_stats(struct context* ctx, struct server* server, struct msg* msg) {
    ASSERT(msg->request);
...
static void req_forward(struct context* ctx, struct conn* c_conn, struct msg* msg) {
    ...
    log_debug(LOG_VERB,
              "forward from c %d to s %d req %" PRIu64 " len %" PRIu32 " type %d with key '%.*s'",
              c_conn->sd,
              s_conn->sd,
              msg->id,
              msg->mlen,
...
struct msg* req_send_next(struct context* ctx, struct conn* conn) {
    ...
    log_debug(LOG_VVERB,
              "send next req %" PRIu64 " len %" PRIu32 " type %d on "
              "s %d",
              nmsg->id,
              nmsg->mlen,
              nmsg->type,
...
void req_send_done(struct context* ctx, struct conn* conn, struct msg* msg) {
    ...
    log_debug(LOG_VVERB,
              "send done req %" PRIu64 " len %" PRIu32 " type %d on "
              "s %d",
              msg->id,
              msg->mlen,
              msg->type,
